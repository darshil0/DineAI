  ...
  return (
    <div className="bg-stone-50 border border-stone-200 rounded-xl p-4 shadow-sm mb-6 max-w-2xl mx-auto">
      ...
      <div className="flex flex-wrap gap-2">
        {hasCuisines &&
          profile.cuisines!.map((cuisine, idx) => (
            <span
              key={idx}
        ...
        {hasAmbiance &&
          profile.ambiance!.map((amb, idx) => (
            <span
              key={idx}
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-stone-200 text-stone-800 text-xs font-medium"
            >
              {amb}
            </span>
          ))}
        ...
        {hasOccasions &&
          profile.special_occasions!.map((occ, idx) => (
            <span
              key={idx}
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-pink-100 text-pink-800 text-xs font-medium"
            >
