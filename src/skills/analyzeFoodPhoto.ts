    ...
    const result = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            cuisines: { type: Type.ARRAY, items: { type: Type.STRING }, description: "Inferred cuisines from the photo" },
            ambiance: { type: Type.ARRAY, items: { type: Type.STRING }, description: "Inferred ambiance or vibe from the photo" },
            description: { type: Type.STRING, description: "A brief 1-sentence description of the food" }
          }
        }
      }
    });

    const output = JSON.parse(cleanJson(result.text || '{"cuisines":[],"ambiance":[],"description":""}'));
    return output;
  },
};
