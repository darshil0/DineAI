...
To handle non-deterministic LLM output (e.g., markdown-wrapped JSON), all parsing must use the `cleanJson` utility from `src/lib/utils.ts` before calling `JSON.parse`.

### 5. Explainability Layer
The system implements a dual-rationale mechanism to ensure transparency:
- **Heuristic Rationale (`whyMatch`)**: Generated by the `scoreRestaurant` skill based on objective constraints (cuisine, price, location).
- **Narrative Rationale (`rationale`)**: Generated by the Finalizer to provide a human-friendly synthesis.
- Both fields are persisted through the `ragRecommender` and displayed in the UI.
