...
- **TypeScript ESM Compliance**: All imports within the `src/` directory must include the `.js` extension (e.g., `import { X } from './Y.js'`) to satisfy Node.js ESM requirements.
- **Multimodal Turns**: Gemini API calls combining text and images must use a single interaction turn structure: `contents: [{ parts: [...] }]`. When using the Gemini SDK, `contents` must be an array of objects and `systemInstruction` must be an object (e.g., `{ parts: [{ text: '...' }] }`).
- **Data Model Integrity**: Use the property `cuisine` (not `cuisine_type`) and a `tags` array for restaurant objects.
- **Google Search Grounding**: Tool configuration for Google Search must use the `googleSearchRetrieval` property with a `dynamicRetrievalConfig` (e.g., `mode: 'MODE_DYNAMIC'`).
...
- **Technology Stack**: Uses React 19, Vite 7, and Tailwind CSS v4. Custom theme variables are defined in `src/index.css` using the `@theme` block.
...
- **Error Handling Architecture**: Wrap skill-level errors in `SkillError` and service-level errors in `AgentServiceError` (from `src/lib/errors.ts`) for consistent propagation.
- **Key Isolation**: RESTRICT all API keys to `server.ts`. Never expose keys to the client.
- **Chat History Validation**: Enforce strict role alternation between `user` and `assistant`. Prevent history from ending with a `user` message to guard against tamper-induced role conflicts.
- **Context Poisoning**: History is truncated to the last 10 exchanges.
- **Cleanup Rules**:
  - Maintain root directory hygiene. Remove unused files like `metadata.json` or redundant logic moved to `src/`.
...
Run built-in tests and system verification to ensure stability:
