...
- **Unused Microphone Permission** (`metadata.json`): `requestFramePermissions` requested `microphone` access, but no documented feature (README, DESIGN.md, SKILLS.md) uses audio input — the app only supports text and photo uploads. Removed the unused permission.
